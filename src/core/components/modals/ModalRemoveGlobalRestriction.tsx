      ...
      <div className="flex flex-col gap-3 mt-2">
        {talentName && (
          <p className="text-sm text-gray-600">
            Talento:{" "}
            <span className="font-semibold text-gray-800">{talentName}</span>
          </p>
        )}
        ...
        <div className="max-h-72 overflow-y-auto rounded-lg border border-gray-200">
          {loadingClients ? (
            <p className="p-3 text-sm text-[#71717A]">Cargando clientes…</p>
          ) : visibleClients.length === 0 ? (
            <p className="p-3 text-sm text-[#71717A]">Sin resultados.</p>
          ) : (
            visibleClients.map((c) => (
              <div key={c.idCliente} className="p-2 border-b last:border-b-0">
