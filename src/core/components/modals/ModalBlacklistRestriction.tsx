      ...
      <div className="flex flex-col gap-4 mt-2">
        {showTalentSearch ? (
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-gray-700">Talento</label>
              ...
              <>
                ...
                {query.trim().length >= 2 && (
                  <div className="mt-1 max-h-44 overflow-y-auto rounded-lg border border-gray-200">
                    {searching ? (
                      <p className="p-3 text-sm text-[#71717A]">Buscando…</p>
                    ) : results.length === 0 ? (
                      <p className="p-3 text-sm text-[#71717A]">
                        Sin resultados.
                      </p>
                    ) : (
                      results.map((t) => (
        ...
        <div className="flex flex-col gap-1">
          ...
          {isEdit ? (
            <input
              ...
              disabled
              className={`${selectClass} bg-gray-100 text-gray-500`}
            />
          ) : (
            <select
              value={idCliente}
