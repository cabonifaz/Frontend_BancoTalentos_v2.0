...
import { ARCHIVO_PDF, DOCUMENTO_CV } from "@/core/utilities/constants";
...
export const ModalUploadResume = ({ idTalento, idArchivo, onUpdate }: Props) => {
    ...
    return (
        <Modal id="modalUploadResume" title="Editar Curriculum Vitae" confirmationLabel="Subir" onConfirm={handleOnConfirm} busy={loading}>
            ...
            <div>
                <h3 className="text-[#71717A] text-sm mt-6 dark:text-slate-400">Sube tu nuevo Curriculum Vitae.</h3>
                <div className="rounded-lg overflow-hidden py-4">
                    <div className="w-full">
                        <div className="relative h-32 rounded-lg border-2 border-gray-100 flex justify-center items-center hover:bg-gray-100 dark:border-slate-700 dark:hover:bg-slate-700">
                            <div className="absolute flex flex-col items-center py-12">
                                ...
                                <span className="text-sm text-[#71717A] mb-6 dark:text-slate-400">{fileName ? "" : "PDF"}</span>
                            </div>
                            <input
                                type="file"
                                name="user-photo"
