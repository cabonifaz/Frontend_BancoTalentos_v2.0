    ...
    <div className="relative flex flex-col gap-1" ref={containerRef}>
      <div className="relative">
        ...
        {open && filtered.length > 0 && (
          <div className="absolute z-20 top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-52 overflow-y-auto">
            ...
            {filtered.map((opt, i) => (
              <button
                key={`${opt.value}-${i}`}
                type="button"
                className="w-full text-left px-3 py-2 hover:bg-gray-50 flex items-start gap-2 border-t border-gray-100 first:border-none transition-colors"
                onClick={() => {
                  onChange(opt.value);
                  setOpen(false);
                }}
