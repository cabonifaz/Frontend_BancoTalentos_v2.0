...
const Campo = ({
  ...
  <label className="flex flex-col gap-1 flex-1 min-w-[110px]">
    <span className="text-[11px] leading-4 uppercase tracking-wide text-gray-400 dark:text-slate-500">
      {etiqueta}
    </span>
    {children}
  </label>
...
const TituloBloque = ({ children }: { children: ReactNode }) => (
  <h3 className="text-[13px] leading-[18px] font-semibold uppercase tracking-wide text-gray-500 mb-2 dark:text-slate-400">
    {children}
  </h3>
);
...
export const ModalCalculadoraRiesgo = ({ onClose }: Props) => {
  ...
  const filaBanda = (grupo: "bandaPlanilla" | "bandaRxh") => (
    <div className="grid grid-cols-[88px_1fr_1fr] gap-x-2 gap-y-1.5 items-center">
      <span />
      <span className="text-[11px] leading-4 uppercase tracking-wide text-gray-400 text-right pr-2 dark:text-slate-500">
        Mínimo
      </span>
      <span className="text-[11px] leading-4 uppercase tracking-wide text-gray-400 text-right pr-2 dark:text-slate-500">
        Máximo
      ...
      {CONCEPTOS.map((concepto) => (
        <div key={concepto.etiqueta} className="contents">
          <span className="text-[13px] text-gray-600 dark:text-slate-300">{concepto.etiqueta}</span>
          <Importe
      ...
      <div className="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto dark:bg-slate-800">
        ...
        <div className="px-6">
          ...
          <div className="py-4 border-b border-gray-100 dark:border-slate-700">
            ...
            <p className="text-xs text-gray-400 mb-3 dark:text-slate-500">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div>
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className="text-[13px] font-medium text-gray-700 dark:text-slate-200">
                    Planilla
                  </span>
                  <div className="w-28">
                    <Selector
