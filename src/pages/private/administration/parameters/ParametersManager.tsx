      ...
      {view === "masters" ? (
        <>
          <header className="mb-5 flex items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Parámetros</h2>
              ...
              <tbody>
                {masters.map((m) => (
                  <tr
                    ...
                    className="border-t border-gray-100 hover:bg-[#009688]/5 cursor-pointer transition-colors"
                  >
                    <td className="px-4 py-2.5 font-medium text-gray-700">{m.idMaestro}</td>
                    <td className="px-4 py-2.5 text-gray-600">{cell(m.descripcion)}</td>
                    <td className="px-4 py-2.5 text-right text-gray-600">{m.totalRegistros}</td>
                    <td className="px-4 py-2.5 text-right text-emerald-600">{m.registrosActivos}</td>
                    <td className="px-4 py-2.5 text-right text-gray-400">{m.registrosInactivos}</td>
                  </tr>
                ))}
          ...
          <header className="mb-5 flex items-start justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <button
                ...
                className="p-2 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-100 transition-colors flex-shrink-0"
                ...
                {items.map((p) => (
                  <tr
                    ...
                    className={`border-t border-gray-100 hover:bg-gray-50 ${
                      p.idEstadoRegistro !== 1 ? "text-gray-400" : "text-gray-600"
                    }`}
                  >
      ...
      {toDelete && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-sm p-6 text-center flex flex-col items-center gap-3">
            ...
            <h3 className="text-base font-semibold text-gray-800">Eliminar parámetro</h3>
            <p className="text-sm text-gray-500">
